-- ============================================================================
-- MboaSchool Pro — Module Emplois du temps
-- ============================================================================
-- MAPPING RÉEL CONFIRMÉ (audit du repo ecoles237-mvp) :
--   - Table `etablissements` -> en réalité `establishments` (owner_id -> auth.uid())
--   - Table `classes` -> existe déjà, on ajoute juste `niveau` si absent
--   - Table `enseignants` -> N'EXISTE PAS, créée ci-dessous
-- ============================================================================

alter table classes add column if not exists niveau text;

-- Table enseignants : n'existait pas dans le schéma actuel, créée ici.
...
-- ----------------------------------------------------------------------------
-- 1. Matières et département disciplinaire
-- ----------------------------------------------------------------------------
...
-- Volume horaire hebdomadaire d'une matière, par niveau (ex: Maths / 6e = 4h)
...
-- ----------------------------------------------------------------------------
-- 2. Rattachement enseignant <-> matières qu'il peut enseigner
-- ----------------------------------------------------------------------------
...
-- Disponibilités hebdomadaires récurrentes d'un enseignant.
-- Si un enseignant n'a AUCUNE ligne ici, il est considéré disponible sur
-- toute l'amplitude horaire de l'établissement (comportement par défaut).
...
-- ----------------------------------------------------------------------------
-- 3. Contraintes de l'établissement (une ligne par établissement)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4. Créneaux horaires — la grille de base générée depuis les contraintes
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5. Emplois du temps — les affectations générées
-- ----------------------------------------------------------------------------
create table if not exists emplois_du_temps (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (classe_id, creneau_id, annee_scolaire),      -- une classe = un seul cours par créneau
  unique (enseignant_id, creneau_id, annee_scolaire)   -- un enseignant = une seule classe par créneau
);
...
-- ============================================================================
-- RLS — chaque établissement ne voit que ses propres données
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- Fonction unique de résolution "quel établissement pour l'utilisateur connecté"
-- ----------------------------------------------------------------------------
-- Confirmé par l'audit de schema.sql : establishments a une colonne owner_id
-- qui référence directement auth.uid() (voir policy "Owners can update own
...
-- matieres : accès direct par etablissement_id
...
-- matieres_volume_horaire : accès via la matière parente
...
-- enseignant_matieres : accès via l'enseignant parent
...
-- enseignant_disponibilites : accès via l'enseignant parent
...
-- enseignants : accès direct par etablissement_id (table nouvellement créée)
